...
$exp_mem_name = "";
$arr_name = array();
$arr_name=$configutil->splash_new($_POST['arr']);
...
$link = mysql_connect(DB_HOST,DB_USER,DB_PWD);
 ...
 /* 存入旧的推广图片更改信息,用于判断是否更改了文字来生成新的推广图片  start*/
$query = "select watertype,exp_pic_text1,exp_pic_text2,exp_pic_text3 from weixin_commonshops where isvalid=true and customer_id=".$customer_id;
$result = mysql_query($query) or die('Query failed: ' . mysql_error());
while ($row = mysql_fetch_object($result)) {
	$old_watertype = $row->watertype;  //选择推广图片风格
	$old_pic_text1 = $row->exp_pic_text1;  //推广图片自定义文字1
	$old_pic_text2 = $row->exp_pic_text2;  //推广图片自定义文字2
	$old_pic_text3 = $row->exp_pic_text3;  //推广图片自定义文字3
}

$query1 = "select shop_id from weixin_commonshop_waters where isvalid=true and customer_id=".$customer_id;
$result1 = mysql_query($query1) or die('Query failed: ' . mysql_error());
$water_shop_id = -1;
while ($row1 = mysql_fetch_object($result1)) {
	$water_shop_id = $row1->shop_id;  
}
if($water_shop_id>0){
	$sql1="update weixin_commonshop_waters set watertype=".$old_watertype.",exp_pic_text1='".$old_pic_text1."',exp_pic_text2='".$old_pic_text2."',exp_pic_text3='".$old_pic_text3."' where shop_id=".$shop_id;

	mysql_query($sql1);
}else{
	$sql1="insert into weixin_commonshop_waters(customer_id,shop_id,watertype,exp_pic_text1,exp_pic_text2,exp_pic_text3,isvalid,createtime)";
	$sql1=$sql1." values(".$customer_id.",".$shop_id.",'".$old_watertype."','".$old_pic_text1."','".$old_pic_text2."','".$old_pic_text3."',true,now())";
